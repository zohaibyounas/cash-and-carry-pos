...
export default function ProductPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch warehouse info for each product
    ...
    const handleEdit = (p: any) => {
        ...
        setFormData({
            ...
            warehouseId: '',
            hasPieces: p.hasPieces || false,
            piecesPerBox: (p.piecesPerBox || 1).toString(),
            pieceCostPrice: (p.pieceCostPrice || 0).toString(),
    ...
    const handleCancel = () => {
        ...
        setEditingId(null);
        setFormData({ 
            name: '', barcode: '', costPrice: '', salePrice: '', discount: '0', vendor: '', category: '', totalStock: '0', warehouseId: '',
            hasPieces: false, piecesPerBox: '1', pieceCostPrice: '0', pieceSalePrice: '0',
            unitName: 'Box', pieceName: 'Piece', initialStockUnit: 'box'
        });
        setFile(null);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!editingId && formData.hasPieces && formData.initialStockUnit === 'box') {
            finalFormData.totalStock = (Number(formData.totalStock) * Number(formData.piecesPerBox)).toString();
        }

        Object.entries(finalFormData).forEach(([key, value]) => data.append(key, value as any));
                ...
                <Card className="animate-in fade-in zoom-in border-slate-200 duration-200 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="barcode" className="flex items-center text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400"><Barcode className="mr-2 h-4 w-4" /> Barcode <span className="ml-1 text-[10px] text-slate-400">(Optional)</span></Label>
                                <Input id="barcode" value={formData.barcode} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" placeholder="Enter barcode (optional)" />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <Input id="totalStock" type="number" value={formData.totalStock} onChange={handleInputChange} className="flex-1 dark:bg-slate-800/50 dark:border-slate-700" required />
                                    {!editingId && formData.hasPieces && (
                                        <select
                                            id="initialStockUnit"
                                            className="w-24 rounded-md border border-input bg-background px-2 py-1 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring dark:border-slate-700 dark:bg-slate-800/50"
                                            value={formData.initialStockUnit}
                                            onChange={handleInputChange}
                                        >
                                            <option value="box">{formData.unitName || 'Box'}</option>
                                            <option value="piece">{formData.pieceName || 'Piece'}</option>
                                        </select>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-2 md:col-span-3 py-2 border-t border-slate-100 dark:border-slate-800 mt-2">
                                ...
                                />
                                <Label htmlFor="hasPieces" className="text-sm font-bold text-slate-700 dark:text-slate-300">
                                    Enable Box/Piece splitting for this product
                                </Label>
                            </div>
                            ...
                            {formData.hasPieces && (
                                <>
                                    <div className="space-y-2">
                                        <Label htmlFor="piecesPerBox" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Pieces per Box</Label>
                                        <Input id="piecesPerBox" type="number" value={formData.piecesPerBox} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" required />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="pieceCostPrice" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Piece Cost Price</Label>
                                        <Input id="pieceCostPrice" type="number" value={formData.pieceCostPrice} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" required />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="pieceSalePrice" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Piece Sale Price</Label>
                                        <Input id="pieceSalePrice" type="number" value={formData.pieceSalePrice} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" required />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="unitName" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Unit Name (e.g. Box/Bore)</Label>
                                        <Input id="unitName" value={formData.unitName} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" required />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="pieceName" className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Sub-unit Name (e.g. Piece/Kg)</Label>
                                        <Input id="pieceName" value={formData.pieceName} onChange={handleInputChange} className="dark:bg-slate-800/50 dark:border-slate-700" required />
                                    </div>
                                </>
                            )}
                                    ...
                                    <div className="p-3">
                                        ...
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <div className="flex shrink-0 flex-col items-end gap-1">
                                                <span className="rounded bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                                                    {p.hasPieces ? (
                                                        <>
                                                            {Math.floor(p.totalStock / (p.piecesPerBox || 1))} {p.unitName || 'Box'}{Math.floor(p.totalStock / (p.piecesPerBox || 1)) !== 0 ? 's' : ''}
                                                            {p.totalStock % (p.piecesPerBox || 1) > 0 && `, ${p.totalStock % (p.piecesPerBox || 1)} ${p.pieceName || 'Piece'}`}
                                                        </>
                                                    ) : (
                                                        <>{p.totalStock} units</>
                                                    )}
                                        ...
                                        {p.warehouses && p.warehouses.length > 0 && (
                                            <p className="mt-1 text-xs text-blue-600 dark:text-blue-400 font-medium">
                                                📦 {p.warehouses.map((w: any) => w.warehouse?.name || 'Unknown').join(', ')}
                                            </p>
                                        )}
