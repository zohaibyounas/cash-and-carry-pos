...
export default function PurchasePage() {
  ...
  const addItem = () => {
    setPurchaseItems([
      ...purchaseItems,
      { product: "", quantity: 1, costPrice: 0, total: 0, unitType: "box" },
    ]);
  };
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    if (field === "quantity" || field === "costPrice") {
      updated[index].total =
        Number(updated[index].quantity) * Number(updated[index].costPrice);
    }
  ...
  const startEdit = (p: any) => {
    ...
    setFormData({
      ...
      paidAmount: (p.paidAmount || 0).toString(),
      balance: (p.balance || 0).toString(),
    });
    setPurchaseItems(p.items ? p.items.map((it: any) => ({ ...it })) : []);
    setFile(null);
    // scroll into view for the add form if needed
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post(`/purchases/${viewingPurchase._id}/payments`, {
            ...
            <html>
                ...
                <body>
                    <div class="center header">
                        ...
                        <p style="margin: 0;">Date: ${formatDateTimeSafe(
                          purchase.createdAt,
                        )}</p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Vendor: {viewingPurchase.vendorName}
              </p>
            </div>
          </div>
          <Button
            onClick={() => handlePrintPurchase(viewingPurchase)}
            className="gap-2 bg-blue-600 font-semibold text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700 shrink-0"
          >
            ...
            <Card className="border-slate-200 dark:border-slate-800 dark:bg-slate-900">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between p-3 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-100 dark:border-green-900/20">
                    ...
                    <span className="font-bold text-green-700">
                      ...
                      {viewingPurchase.totalAmount -
                        viewingPurchase.balance -
                        (viewingPurchase.paymentHistory?.reduce(
                          (s: any, p: any) => s + p.amount,
                          0,
                        ) || 0)}
                    </span>
                  </div>
                  {viewingPurchase.paymentHistory?.map(
                    (p: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex justify-between p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20"
                      >
                        <div className="flex items-center gap-3">
                          <Calendar className="h-4 w-4 text-blue-600" />
                          <span className="text-sm font-medium">
                            {formatDateTimeSafe(p.date)}
                          </span>
                        </div>
                  ...
                  {(!viewingPurchase.paymentHistory ||
                    viewingPurchase.paymentHistory.length === 0) && (
              ...
              <CardContent className="p-6 space-y-4">
                <div>
                  ...
                  <p className="text-3xl font-black italic tracking-tighter">
                    Rs. {viewingPurchase.totalAmount.toLocaleString()}
                  </p>
                </div>
                <div className="flex justify-between">
                  <div>
                    ...
                    <p className="text-xl font-bold">
                      Rs. {viewingPurchase.paidAmount.toLocaleString()}
                    </p>
                  </div>
            ...
            {viewingPurchase.billImage && (
              <Card className="overflow-hidden border-slate-200 dark:border-slate-800 dark:bg-slate-900">
                ...
                <CardContent className="p-4">
                  <img
                    src={`http://localhost:5000${viewingPurchase.billImage}`}
                    alt="Bill"
                    className="w-full h-auto rounded-lg border dark:border-slate-800"
                  />
                </CardContent>
                <CardFooter>
                  <Button variant="outline" className="w-full gap-2" asChild>
                    <a
                      href={`http://localhost:5000${viewingPurchase.billImage}`}
                      target="_blank"
                    >
                      View Full Size
                    </a>
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Track vendor shipments and historical payments.
          </p>
        </div>
            ...
            <CardContent className="pt-4">
              ...
              >
                ...
                <Button
                  size="sm"
                  type="submit"
                  className="h-9 gap-2 bg-blue-600 font-semibold text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700"
                >
                  Filter
                </Button>
              </form>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    required
                    className="dark:bg-slate-800/50 dark:border-slate-700"
                    placeholder="e.g. Master Sanitary Ltd."
                  />
                </div>
                <div className="space-y-2">
                  <Label
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center gap-4">
                  <h3 className="font-black uppercase tracking-widest text-xs text-blue-600">
                    Product Line Items
                  </h3>
                ...
                <div className="space-y-3">
                  {purchaseItems.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="group grid grid-cols-1 items-center gap-4 rounded-xl border border-slate-200 p-3 dark:border-slate-800 dark:bg-slate-800/30 md:grid-cols-12"
                      >
                        ...
                        <div className="md:col-span-1 space-y-1">
                          ...
                          <select
                            className="h-9 w-full rounded-md border border-input bg-background px-2 py-1 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring dark:border-slate-700 dark:bg-slate-800/50"
                            value={item.unitType || "box"}
                            onChange={(e) =>
                              updateItem(idx, "unitType", e.target.value)
                            }
                          >
                            <option value="box">
                              {prod?.unitName || "Box"}
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-9 w-9 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 dark:hover:text-red-400"
