...
export default function PurchasePage() {
    ...
    const addItem = () => {
        setPurchaseItems([...purchaseItems, { product: '', quantity: 1, costPrice: 0, total: 0, unitType: 'box' }]);
    };
            ...
            <html>
                ...
                <body>
                    <div class="center header">
                        ...
                        <p style="margin: 0;">Date: ${formatDateTimeSafe(purchase.createdAt)}</p>
    ...
    const addProductToPurchase = (prod: any) => {
        ...
        const newItem = { product: prod._id, quantity: 1, costPrice: prod.costPrice || 0, total: prod.costPrice || 0, unitType: 'box' };
                                        ...
                                        {viewingPurchase.items?.map((item: any, idx: number) => {
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{item.quantity} {item.unitType === 'piece' ? (prod?.pieceName || 'Piece') : (prod?.unitName || 'Box')}</td>
                                    ...
                                    {viewingPurchase.paymentHistory?.map((p: any, idx: number) => (
                                        <div key={idx} className="flex justify-between p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-medium">{formatDateTimeSafe(p.date)}</span>
                                            </div>
                                        ...
                                        return (
                                            <div key={idx} className="group grid grid-cols-1 items-center gap-4 rounded-xl border border-slate-200 p-3 dark:border-slate-800 dark:bg-slate-800/30 md:grid-cols-12">
                                                ...
                                                <div className="md:col-span-1 space-y-1">
                                                    ...
                                                    >
                                                        <option value="box">{prod?.unitName || 'Box'}</option>
                                                        {prod?.hasPieces && <option value="piece">{prod?.pieceName || 'Piece'}</option>}
                                                    </select>
                                                </div>
                                                <div className="md:col-span-2 space-y-1">
                                                    <Label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Qty ({item.unitType === 'piece' ? (prod?.pieceName || 'pcs') : (prod?.unitName || 'units')})</Label>
                                                    <Input type="number" step="any" min="0" value={item.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} required className="h-9 dark:bg-slate-800/50 dark:border-slate-700" />
                                                </div>
                                                <div className="md:col-span-2 space-y-1">
                                                    <Label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Cost Price</Label>
                                ...
                                {purchases.length === 0 ? (
                                    ...
                                    purchases.map((p) => (
                                        <tr key={p._id} className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{formatDateTimeSafe(p.createdAt)}</td>
