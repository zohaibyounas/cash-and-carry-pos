          ...
          storesArr.map(async (store: any) => {
            try {
              ...
              const totalStock = products.reduce(
                (sum: number, p: any) => sum + (Number(p.totalStock) || 0),
                0,
              );
              const totalCostValue = products.reduce((sum: number, p: any) => {
                const stock = Number(p.totalStock) || 0;
                const perPieceCost = p.hasPieces
                  ? Number(p.pieceCostPrice) ||
                    Number(p.costPrice) / (Number(p.piecesPerBox) || 1)
                  : Number(p.costPrice);
                return sum + stock * perPieceCost;
              }, 0);
              const totalSaleValue = products.reduce((sum: number, p: any) => {
                const stock = Number(p.totalStock) || 0;
                const perPieceSale = p.hasPieces
                  ? Number(p.pieceSalePrice) ||
                    Number(p.salePrice) / (Number(p.piecesPerBox) || 1)
                  : Number(p.salePrice);
              ...
              console.error(
                `Failed to fetch products for store ${store.name}`,
                error,
              );
              return {
                store,
          ...
          >
            <CardHeader>
              ...
              <CardDescription>
                ...
                {storeData.stats.totalStock} Total Items
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Statistics Grid */}
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {storeData.products.map((product: any) => {
                        ...
                        const perPieceCost = product.hasPieces
                          ? Number(product.pieceCostPrice) ||
                            Number(product.costPrice) /
                              (Number(product.piecesPerBox) || 1)
                          : Number(product.costPrice);
                        const perPieceSale = product.hasPieces
                          ? Number(product.pieceSalePrice) ||
                        ...
                        return (
                          ...
                          >
                              ...
                              >
                                {product.hasPieces
                                  ? `${Math.floor(
                                      ...
                                      stock % (product.piecesPerBox || 1) > 0
                                        ? `, ${
                                            stock % (product.piecesPerBox || 1)
                            ...
                            <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-400">
                              Rs. {perPieceCost.toLocaleString()}
                            </td>
                            <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-400">
                              Rs. {perPieceSale.toLocaleString()}
                            </td>
